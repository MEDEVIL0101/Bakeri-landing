...
**Blanket sweep (2026-08-28, at Diana's request — Cookiesbysteph is the only baker who has sold digital):** re-issued fresh 1-year links for **all 78 of her non-test digital orders** (`resend-digital-download` per `order_id`; the ~12 buyers already handled individually earlier that day got a harmless duplicate). 3 single-item orders and 13 multi-item orders initially dropped a line for **"Dumpling Surprise Mystery Bag Printable | 8.5x11 Foldable…"** (and one "Zombie Snack Attack…") because each name maps to two of her listings with different files (accidental PDF + PNG double-uploads on 08-24). Fixed two ways: the 3 single-item orders had their line pointed at the PDF listing; and `resend-digital-download`'s resolver now, when a name matches multiple listings with **different** files, hands over **every** candidate file rather than dropping the line (still collapses when the matches share one file). All 16 re-sent clean.

**Duplicate listings resolved (2026-08-28, Diana's call):** the "Mystery Bag" and "Zombie Snack Attack" pairs were literal duplicates — identical title, identical description (both say "PDF"), same $2 price, `has_variants=false`, differing only in the uploaded file (a PDF and a PNG created ~1 min apart; sales had been splitting ~random between them). Kept the PDF listing of each, soft-deleted + unlisted the PNG one (`a43117fa`, `7b5d64ec`), repointed their `order_items.menu_item_id` to the kept PDF listing (0 dangling refs after), left the PNG files in storage so links already sent today keep working. Cookiesbysteph now has no exact-name duplicate digital listings. Also renamed the listing literally titled `"Title: Cute Skeleton Face PNG | Halloween Digital Download"` → `"Cute Skeleton Face PNG | Halloween Digital Download"` (24 referencing orders repointed by `menu_item_id` first so the rename doesn't break name resolution). Still outstanding on her side: update her app.
