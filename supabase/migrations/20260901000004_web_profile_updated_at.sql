-- Surface profiles.updated_at in the web profile RPCs so baker/index.html can
-- cache-bust storefront images (header, logo, about portrait).
--
-- Problem: those images live at fixed Storage paths (<uid>/header.jpg etc.) and
-- the storefront loads them with no version query param. On publish the bytes
-- at that path are replaced, but any browser that loaded the storefront in the
-- last hour keeps serving the stale image from its own cache (Storage stamps a
-- default max-age=3600). Appending ?v=<updated_at epoch> lets the page bust the
-- cache exactly when the baker republishes and otherwise cache normally.
--
-- publishStorefront() already bumps profiles.updated_at on every publish (the
-- store_policies patch), so it's a reliable change token.
--
-- Both get_baker_web_profile_by_* bodies are otherwise byte-identical to
-- 20260828000006 — the only change is `updated_at` added to the v_profile
-- subquery (same expression in both functions).
...
BEGIN
    ...
    FROM (
        SELECT
            ...
            shipping_additional_item_percent, updated_at,
