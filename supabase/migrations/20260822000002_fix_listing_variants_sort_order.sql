-- HOTFIX: 20260822000001_listing_variants.sql's listing_variants subquery
-- selected id/label/price/stock_qty but not sort_order, while the outer
-- json_agg(o ORDER BY o.sort_order) referenced it — "column t.sort_order
-- does not exist" on every single call to both web-profile RPCs, taking
-- down every baker's storefront page load. Adds sort_order to the
-- projection; otherwise identical to the previous migration.
    ...
    FROM (
        SELECT
            ...
            (SELECT json_agg(o ORDER BY o.sort_order) FROM (
                SELECT id, label, price, stock_qty, sort_order
