-- Storefront-wide "free shipping over $X" rule, layered on top of each
-- physical listing's own flat shipping_fee (20260821000001). 0 means no
-- rule — same convention as the existing (currently unused-in-storefront)
-- delivery_free_over column. Per-region rates aren't built; this is
-- intentionally just the one flat threshold.

ALTER TABLE public.profiles
    ADD COLUMN IF NOT EXISTS shipping_free_over_threshold NUMERIC NOT NULL DEFAULT 0;

-- MARK: web-profile RPCs — add shipping_free_over_threshold to the public
-- profile payload so the storefront can waive shipping fees at checkout.
--
-- Purely additive — everything else about these two functions is
-- unchanged from 20260821000001_physical_shipping_fee.sql.
