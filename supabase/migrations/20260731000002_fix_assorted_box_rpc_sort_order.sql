-- Hotfix: 20260731000001_assorted_box.sql referenced t.sort_order / v.sort_order
-- in the size_tiers/variants correlated subqueries' ORDER BY without including
-- sort_order in the inner SELECT list, breaking get_baker_web_profile_by_slug
-- and get_baker_web_profile_by_id for every baker (error 42703), not just
-- Assorted Box listings. Fix: include sort_order in the inner SELECT.
