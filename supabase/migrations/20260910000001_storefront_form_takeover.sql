-- Storefront "custom form takeover" mode.
--
-- A baker who exclusively does custom orders (e.g. a high-volume caterer who
-- today links straight to a Google Form from her bio) can mark one custom
-- listing as her whole storefront. When set, baker/index.html renders just the
-- header + About (if she's written one) and hands the body off to the intake
-- form — no menu grid, no cards.
--
-- profiles.storefront_listing_id points at that custom listing (public.menu_items).
-- The listing carries its own intake_form_id, so the web page needs nothing more
-- than this one column to resolve the form. ON DELETE SET NULL so deleting the
-- listing cleanly drops the storefront back to the normal grid.
--
-- The Market tab (iOS) enforces the other direction: it clears this column when
-- the baker adds / re-shows any market item, so the two layouts stay mutually
-- exclusive with a single source of truth.
--
-- Both get_baker_web_profile_by_* bodies below are otherwise byte-identical to
-- 20260901000004 — the only change is `storefront_listing_id` added to the
-- v_profile subquery (same expression in both functions).

ALTER TABLE public.profiles
    ADD COLUMN IF NOT EXISTS storefront_listing_id UUID
    REFERENCES public.menu_items(id) ON DELETE SET NULL;

COMMENT ON COLUMN public.profiles.storefront_listing_id IS
    'When set, the storefront renders as a single custom-order form (the intake form on this menu_items row) instead of the normal listing grid. Mutually exclusive with having visible market items.';
