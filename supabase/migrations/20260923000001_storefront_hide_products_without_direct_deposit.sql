...
-- 20260715000006_web_profile_stripe_gate.sql — the payment functions
-- reject it and baker/index.html greys out every buy button as "Ordering
-- isn't live yet"). This goes one step further: the products aren't
-- returned to the public page at all, so an unconnected shop shows only
-- what actually works instead of a grid of disabled items. The vendor
-- themself still gets their full listings back (auth.uid() = owner), so
-- the iOS builder preview keeps showing products they've added while
