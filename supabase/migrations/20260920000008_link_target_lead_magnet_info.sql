-- Bug fix: a baker_links row with target_type='product' pointing at a
-- lead-magnet listing (created via the iOS "Upload a File" shortcut) was
-- unreachable on the storefront — v_links' target listing lookup expected
-- the web page to cross-reference allListings (v_listings), but
-- v_listings deliberately excludes lead magnets (is_listed_in_marketplace
-- stays false for them). The web's "listing not found" fallback then
-- rendered the link disabled instead of broken, per the original design
-- note — but that meant a lead-magnet-targeted link card never worked at
-- all, not a graceful degradation.
--
-- Fix: embed the target listing's name + is_lead_magnet flag directly on
-- each v_links row (LEFT JOIN, regardless of marketplace-listed status),
-- so the web renderer needs no cross-reference at all — a product-target
-- link now always has everything it needs on the row itself, whether the
-- target is a normal listed item or a lead magnet.
