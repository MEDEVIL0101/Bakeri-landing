-- Manual flat shipping fee for physical listings — a placeholder ahead of
-- real carrier-rate integration (USPS/UPS/Canada Post APIs), which needs a
-- separate application/approval process with each carrier and isn't built
-- yet. For now a baker just sets a flat dollar amount per listing, added to
-- the buyer's total at checkout the same way delivery_fee already works for
-- local delivery on food items.

ALTER TABLE public.menu_items
    ADD COLUMN IF NOT EXISTS shipping_fee NUMERIC NOT NULL DEFAULT 0;

-- MARK: web-profile RPCs — add shipping_fee to the public listings feed
--
-- Purely additive — everything else about these two functions is
-- unchanged from 20260820000001_physical_products.sql (which added
-- lead_days the same way).
