...
// Called directly by mark-order-delivered, same shape/shell as
// send-guest-order-shipped-email (which it's modeled on) — no tracking
// block here since the shipped email already surfaced that; this one just
// confirms arrival.
...
Deno.serve(async (req: Request) => {
  ...
  try {
    const { data: order, error: orderErr } = await db
      .from("orders")
      .select("id, order_name, customer_name, customer_email, user_id, delivered_at, invoice_code")
      .eq("id", orderId)
      .single();
    ...
    const arrivedHtml = `
      <div style="margin:18px 0;padding:18px 16px;background:#F7F2E9;border-radius:10px;text-align:center;">
        <div style="font-size:11.5px;font-weight:700;letter-spacing:.02em;color:#A89B8C;text-transform:uppercase;">Delivered</div>
        <div style="font-size:22px;font-weight:700;color:#241712;margin-top:6px;">Your order has arrived!</div>
      </div>
    `;
    ...
    const legalHtml = `
      <p style="color:#A89B8C;font-size:11.5px;line-height:1.5;margin-top:20px;">
        Any issue with your order? Reply to this email or use the contact info above — either way it reaches ${escapeHtml(bakerName)} directly.
      </p>
    `;
    ...
    const html = renderReceiptShell({
      ...
      bakerUrl,
      metaRowsHtml: [
        metaRow("", escapeHtml(formatDate(order.delivered_at ?? new Date().toISOString()))),
    ...
    const resendRes = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        ...
        subject: "Your order has arrived — " + (order.order_name || "your order"),
        html,
      }),
    });
    ...
    await logNotification(db, orderId, "guest_order_delivered", "sent");
    return json({ ok: true });
  } catch (err: unknown) {
    const message = err instanceof Error ? err.message : String(err);
    console.error(`send-guest-order-delivered-email failed for order ${orderId}:`, message);
    await logNotification(db, orderId, "guest_order_delivered", "failed", message.slice(0, 500));
