  ...
  try {
    ...
    const resendRes = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        ...
        subject: "Your order has shipped — " + (order.order_name || "your order"),
        html,
      }),
    });
    ...
    await logNotification(db, orderId, "guest_order_shipped", "sent");
    return json({ ok: true });
  } catch (err: unknown) {
    const message = err instanceof Error ? err.message : String(err);
    console.error(`send-guest-order-shipped-email failed for order ${orderId}:`, message);
    await logNotification(db, orderId, "guest_order_shipped", "failed", message.slice(0, 500));
