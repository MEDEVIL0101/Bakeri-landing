// Shared HMAC signing/verification for the Square OAuth `state` param.
// Used by square-oauth-start (signs) and square-oauth-callback (verifies) so
// the callback can trust the userId embedded in state actually came from an
// authenticated request, not just whatever the browser redirect carried.
//
// Format: "{userId}.{expiresAtEpochSeconds}.{base64url HMAC-SHA256 signature}"

const SECRET = Deno.env.get("SQUARE_OAUTH_STATE_SECRET")!;
